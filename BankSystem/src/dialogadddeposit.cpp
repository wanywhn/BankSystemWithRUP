...
void DialogAddDeposit::init_ui() {
  ...
  connect(cb_deposit_kind, static_cast<void (QComboBox::*)(int)>(
                               &QComboBox::currentIndexChanged),
          this, [this, common](int index) {
            ...
            if (index == 1) {
              if (last == 1) {
                ...
                cb_cunqi->addItems({"1", "5"});
                rb_autocontinue = new QRadioButton;
                connect(cb_cunqi, static_cast<void (QComboBox::*)(int)>(
                                      &QComboBox::currentIndexChanged),
                        this, [this](int index) {
                          this->lb_lilv->setText(
                              QString::number(ctrl.get_lilv(index + 1)));
                        });

                common->insertRow(4, "Cunqi", cb_cunqi);
                common->insertRow(4, "Auto Continue", rb_autocontinue);
                last = 1;
                lb_lilv->setText(QString::number(ctrl.get_lilv(1)));
              }
            } else {
              if (last == 1) {
  ...
  connect(btn_accept, &QPushButton::clicked, this,
          [this]() { this->process_btn(cb_deposit_kind->currentIndex()); });
  connect(btn_calcen, &QPushButton::clicked, this,
          [this]() { this->reject(); });
}
